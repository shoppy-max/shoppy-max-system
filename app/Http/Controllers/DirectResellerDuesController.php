...
class DirectResellerDuesController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        return view('direct-resellers.dues.index', compact('resellers'));
    }
    ...
    {
        $reseller = Reseller::direct()->findOrFail($id);
        ...
        $orderDescriptionSql = in_array($driver, ['mysql', 'mariadb'], true)
            ? 'CONCAT("Order placed (", status, ")")'
            : "'Order placed (' || status || ')'";
        $paymentDescriptionSql = in_array($driver, ['mysql', 'mariadb'], true)
            ? 'CONCAT("Payment received (", payment_method, ")")'
        ...
        $ordersQuery = \DB::table('orders')
            ->select(
                ...
                \DB::raw($orderDescriptionSql.' as description'),
        ...
        $paymentsQuery = \DB::table('reseller_payments')
            ->select(
                ...
                \DB::raw($paymentDescriptionSql.' as description'),
                'id as original_id'
            )
            ->where('reseller_id', $id);
        ...
        $paymentsInRange = \DB::table('reseller_payments')
            ->where('reseller_id', $id)
        ...
        $statement = $transactions->map(function ($t) use (&$running) {
            ...
            if ($t->type === 'Order') {
                ...
                $t->url = route('direct-reseller-payments.edit', $t->original_id);
        ...
        return view('direct-resellers.dues.show', compact('reseller', 'statement', 'transactions', 'balanceForward', 'startDate', 'endDate'));
    }
}
