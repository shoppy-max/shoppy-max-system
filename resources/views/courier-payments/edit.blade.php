<x-app-layout>
    ...
    <form method="POST" action="{{ route('courier-payments.update', $courierPayment) }}">
        @csrf
        @method('PUT')
        <div class="max-w-4xl mx-auto p-6 space-y-6">
            ...
            <div class="p-6 bg-white rounded-lg shadow dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div x-data="{ 
                        courierSearch: '{{ $courierPayment->courier->name }}', 
                        courierOpen: false, 
                        selectedCourier: {{ json_encode(['id' => $courierPayment->courier->id, 'name' => $courierPayment->courier->name]) }},
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" step="0.01" min="0" value="{{ old('amount', $courierPayment->amount) }}" required
                               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                               placeholder="0.00">
                        @error('amount')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="payment_note" rows="3"
                                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                                  placeholder="Additional notes about this payment...">{{ old('payment_note', $courierPayment->payment_note) }}</textarea>
                        @error('payment_note')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
