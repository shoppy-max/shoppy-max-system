<div
    x-cloak
    x-show="bulkReprintWaybillModalOpen"
    x-transition.opacity
    @keydown.escape.window="closeBulkReprintWaybillModal()"
    class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"
>
    <div
        @click.outside="closeBulkReprintWaybillModal()"
    ...
    >
        <div class="flex items-start justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-700">
            <div>
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Reprint Selected Waybills</h3>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    <span class="font-medium text-gray-900 dark:text-gray-100" x-text="selectedReprintableWaybillCount()"></span>
                    selected orders already have saved waybills. Reprint uses those saved IDs only.
                </p>
                <template x-if="selectedOrders.length > selectedReprintableWaybillCount()">
                    <p class="mt-1 text-xs text-amber-600 dark:text-amber-300">
                        Orders without saved waybills are excluded from bulk reprint.
                    </p>
                </template>
            </div>
            <button
                type="button"
                @click="closeBulkReprintWaybillModal()"
        ...
        <div class="grid grid-cols-1 gap-4 p-6 md:grid-cols-2">
            <button
                type="button"
                @click="submitBulkWaybillReprint('a4')"
            ...
            >
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Reprint in a 2 x 2 A4 grid using the saved waybill IDs.</p>
                    </div>
            ...
            <button
                type="button"
                @click="submitBulkWaybillReprint('four_by_six')"
                class="rounded-2xl border border-gray-200 bg-gray-50 p-5 text-left transition hover:border-blue-400 hover:bg-blue-50 dark:border-gray-700 dark:bg-gray-900/20 dark:hover:border-blue-500 dark:hover:bg-blue-900/20"
            >
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <p class="text-base font-semibold text-gray-900 dark:text-white">4 x 6 Size</p>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Reprint one industrial courier waybill per label page.</p>
                    </div>
