    ...
    <div class="p-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-gray-800">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0 md:space-x-4 mb-6">
            ...
            <div class="flex-1 w-full md:max-w-lg">
                <form method="GET" action="{{ route('resellers.index') }}" class="relative">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <a href="{{ route('resellers.create') }}" class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800 transition-transform transform hover:scale-105 shadow-lg">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                    Add Direct Reseller
                </a>

                <div class="flex items-center gap-2">
                     <a href="{{ route('resellers.index', array_merge(request()->all(), ['export' => 'excel'])) }}" class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-green-500 dark:hover:bg-green-600 dark:focus:ring-green-800 flex items-center shadow-md transition-transform transform hover:scale-105">
                ...
                <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th scope="col" class="px-6 py-3">
                             <a href="{{ route('resellers.index', array_merge(request()->all(), ['sort' => 'business_name', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc'])) }}" class="flex items-center group cursor-pointer hover:text-primary-600 dark:hover:text-white">
                                Business
                        ...
                        <th scope="col" class="px-6 py-3">
                            <a href="{{ route('resellers.index', array_merge(request()->all(), ['sort' => 'name', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc'])) }}" class="flex items-center group cursor-pointer hover:text-primary-600 dark:hover:text-white">
                                Contact Person
                        ...
                        <th scope="col" class="px-6 py-3">
                             <a href="{{ route('resellers.index', array_merge(request()->all(), ['sort' => 'due_amount', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc'])) }}" class="flex items-center group cursor-pointer hover:text-primary-600 dark:hover:text-white">
                                Due Amount
                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    <a href="{{ route('resellers.show', $reseller) }}" class="p-2 text-blue-600 hover:bg-blue-100 rounded-lg dark:text-blue-400 dark:hover:bg-gray-700 transition-colors" title="View">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                    </a>
                                    <a href="{{ route('resellers.edit', $reseller) }}" class="p-2 text-yellow-600 hover:bg-yellow-100 rounded-lg dark:text-yellow-400 dark:hover:bg-gray-700 transition-colors" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <form
                                        action="{{ route('resellers.reset-password', $reseller) }}"
                                        method="POST"
                                    ...
                                    <form action="{{ route('resellers.destroy', $reseller) }}" method="POST" data-confirm-message="Are you sure you want to delete this direct reseller?" class="inline">
                                        @csrf
                                        @method('DELETE')
