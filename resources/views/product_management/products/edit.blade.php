<x-app-layout>
    <x-form-layout>
        ...
        @php
            ...
            $defaultVariants = $product->variants->map(function ($variant) {
                return [
                    'id' => $variant->id,
            ...
            $initialVariants = collect(old('variants', $defaultVariants))->map(function ($variant) use ($canManageDirectProductPrices, $canManageResellerProductPrices) {
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" @submit="handleSubmit"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <!-- Basic Information -->
                    <x-form-section title="Basic Information" description="Update the core product details.">
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" id="name" name="name" value="{{ old('name', $product->name) }}" x-model="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="e.g. Wireless Headphones" required>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-2">
                                        <select id="sub_category_id" name="sub_category_id" x-model="selectedSubCategory" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">
                                    ...
                                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400" x-show="!selectedCategory">Select a category first to add a sub-category.</p>
                                </div>
                            </div>
                            
                            <div>
                                <label for="description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Description</label>
                                <textarea id="description" name="description" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Product description...">{{ old('description', $product->description) }}</textarea>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="number" id="warranty_period" name="warranty_period" value="{{ old('warranty_period', $product->warranty_period) }}" min="0" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="e.g., 12">
                                ...
                                <div>
                                    ...
                                    <select id="warranty_period_type" name="warranty_period_type" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                        ...
                                        <option value="years" {{ old('warranty_period_type', $product->warranty_period_type) == 'years' ? 'selected' : '' }}>Years</option>
                                        <option value="months" {{ old('warranty_period_type', $product->warranty_period_type) == 'months' ? 'selected' : '' }}>Months</option>
                                        <option value="days" {{ old('warranty_period_type', $product->warranty_period_type) == 'days' ? 'selected' : '' }}>Days</option>
                                    </select>
                                ...
                                <div class="p-4 border border-gray-200 rounded-lg dark:border-gray-700 bg-gray-50 dark:bg-gray-800 relative">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="flex">
                                                 <span @click="generateSmartSku(index, true)" class="cursor-pointer inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 border border-r-0 border-gray-300 rounded-l-md dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600 hover:bg-gray-300" title="Click to Regenerate">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
                                                </span>
                                                <input type="text" :id="'variant_sku_'+index" :name="'variants['+index+'][sku]'" x-model="variant.sku" class="rounded-none rounded-r-lg bg-gray-100 border border-gray-300 text-gray-700 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 cursor-not-allowed" readonly required>
                                            </div>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">SKU is auto-generated. Click the refresh icon to regenerate.</p>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <input type="number" :id="'variant_qty_'+index" :name="'variants['+index+'][quantity]'" x-model="variant.quantity" readonly class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 cursor-not-allowed dark:bg-gray-600 dark:border-gray-600 dark:text-white">
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Stock is managed automatically from stock/order flows.</p>
                                        </div>
                                         
                                        <!-- Alert Quantity -->
                                         <div>
                                            <label :for="'variant_alert_'+index" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Alert Qty</label>
                                            <input type="number" :id="'variant_alert_'+index" :name="'variants['+index+'][alert_quantity]'" x-model="variant.alert_quantity" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">
                                        </div>
                        ...
                        <div x-data="imageViewer()">
                            ...
                            <div class="flex items-center justify-center w-full">
                                <label for="image" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600 relative overflow-hidden group">
                                    ...
                                    <!-- Placeholder (Show if no URL and no current image) -->
                                    <div class="flex flex-col items-center justify-center pt-5 pb-6" x-show="!imageUrl && !currentImage">
                                    ...
                                    <img x-show="imageUrl || currentImage" :src="imageUrl || currentImage" class="absolute inset-0 w-full h-full object-cover" />
                                    
                                    <!-- Overlay on Hover (when image exists) -->
                                    <div x-show="imageUrl || currentImage" class="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    <script>
        function productForm(backendUnits, oldVariants) {
            return {
                ...
                newSubCategories: [],
                newUnits: [],
                unitsList: backendUnits,

                // Form Data (Main)
                name: '{{ old('name', $product->name) }}',

                // Variants Data
                // If oldVariants is empty (should not happen if edit logic is correct for existing items), init with one.
                variants: (oldVariants && oldVariants.length) ? oldVariants : [{id: null, unit_id: '', unit_value: '', sku: '', selling_price: '', limit_price: '', quantity: 0, alert_quantity: 5}],
                ...
                init() {
                     ...
                     if(!this.variants || this.variants.length === 0) {
                        this.variants = [];
                        this.addVariant();
                     } else if (this.variants.length === 1 && !this.variants[0].sku) {
                ...
                generateSmartSku(index, force = false) {
                     ...
                     // Safety: Don't auto-update existing variants (those with IDs) unless forced (button click)
                     if (variant.id && !force) return;
                ...
                async saveSubCategory() {
                    try {
                        ...
                        if(response.data.success) {
                            ...
                            this.selectedSubCategory = response.data.subCategory.id;
