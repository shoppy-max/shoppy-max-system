<x-app-layout>
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Edit Direct Payment</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h2 class="mt-4 text-2xl font-bold text-gray-900 dark:text-white">Edit Direct Payment</h2>
        </div>

        <form method="POST" action="{{ route('direct-reseller-payments.update', $resellerPayment) }}" x-data="paymentForm()">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <x-form-section title="Payment Details" description="Update payment information.">
                        <div class="space-y-6">
                            <!-- Reseller Select (Searchable) - Disabled in Edit for safety or allowed? Usually safer to keep fixed or handle carefully. Logic handles reseller change but complexity increases. Let's allow but warn. -->
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" id="amount" name="amount" x-model="amount" value="{{ old('amount', $resellerPayment->amount) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required placeholder="0.00">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select id="payment_method" name="payment_method" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required>
                                        <option value="bank" {{ (old('payment_method') ?? $resellerPayment->payment_method) == 'bank' ? 'selected' : '' }}>Bank Transfer</option>
                                        <option value="cash" {{ (old('payment_method') ?? $resellerPayment->payment_method) == 'cash' ? 'selected' : '' }}>Cash</option>
                                        <option value="other" {{ (old('payment_method') ?? $resellerPayment->payment_method) == 'other' ? 'selected' : '' }}>Other</option>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input type="date" id="payment_date" name="payment_date" value="{{ old('payment_date', $resellerPayment->payment_date->format('Y-m-d')) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required>
                            ...
                            <div>
                                ...
                                <input type="text" id="reference_id" name="reference_id" value="{{ old('reference_id', $resellerPayment->reference_id) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="Cheque number, transaction ID, etc.">
                ...
                <!-- Right Column (1/3) -->
                <div class="space-y-6">
                    <x-form-section title="Balance Adjustment">
                        <div class="space-y-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <div class="flex justify-between items-center pb-2 border-b border-gray-200 dark:border-gray-600">
                                <span class="text-sm text-gray-600 dark:text-gray-300">Original Amount:</span>
                                <span class="text-lg font-bold text-gray-900 dark:text-white" x-text="'Rs. ' + originalAmount.toFixed(2)"></span>
                            </div>
                            ...
                            <div class="flex justify-between items-center pt-2 border-t border-gray-200 dark:border-gray-600">
                                <span class="text-sm font-semibold text-gray-900 dark:text-white">Difference:</span>
                                <span class="text-xl font-bold" :class="calculateDiff() > 0 ? 'text-green-600' : 'text-red-600'" x-text="'Rs. ' + calculateDiff()"></span>
                            </div>
                        </div>
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <a href="{{ route('direct-reseller-payments.index') }}" class="w-full py-2.5 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 text-center">
                            Cancel
                        </a>
                    </div>
                </div>
    ...
    <script>
        function paymentForm() {
            return {
                currentDue: 0,
                amount: '{{ old('amount', $resellerPayment->amount) }}',
                originalAmount: {{ $resellerPayment->amount }},
                ...
                calculateDiff() {
                    let oldVal = parseFloat(this.originalAmount);
                    let newVal = parseFloat(this.amount) || 0;
