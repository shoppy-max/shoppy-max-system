    ...
    <div class="p-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-gray-800" x-data="{ selected: [], allSelected: false, toggleAll() { this.allSelected = !this.allSelected; if(this.allSelected) { this.selected = [{{ $payments->pluck('id')->implode(',') }}]; } else { this.selected = []; } } }">
        ...
        <div class="flex flex-col space-y-4 lg:flex-row lg:items-start lg:justify-between lg:space-y-0 lg:space-x-4 mb-6">
            ...
            <div class="w-full lg:flex-1">
                <form method="GET" action="{{ route('direct-reseller-payments.index') }}">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
                        ...
                        <div class="md:col-span-2 flex items-center gap-2">
                            ...
                            @if(request('search') || request('start_date') || request('end_date'))
                                <a href="{{ route('direct-reseller-payments.index') }}" class="w-full md:w-auto text-center p-2.5 text-sm font-medium text-gray-900 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Clear</a>
                            @endif
                        </div>
                    </div>
                </form>
